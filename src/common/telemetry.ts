...
export function sendTensorboardPromptSelection(selection: TensorBoardPromptSelection) {
  publicLog2<TensorboardSelectionData, TensorboardSelectionDataClassification>('TENSORBOARD.INSTALL_PROMPT_SELECTION', {
    selection,
  });
}
...
export function sendTensorboardLaunch(entrypoint: TensorBoardEntrypoint, trigger: TensorBoardEntrypointTrigger) {
  publicLog2<TensorboardLaunchData, TensorboardLaunchDataClassification>('TENSORBOARD.SESSION_LAUNCH', {
    trigger,
    entrypoint,
  });
}
...
export function sendJumptToSource() {
  publicLog2<JumptToSourceData, JumptToSourceDataClassification>('TENSORBOARD_JUMP_TO_SOURCE_REQUEST');
}
